import {cyanBright, green, magentaBright, red, xterm} from 'cli-color';

import {isObject} from './shared';

declare const process: any;
const yellow = xterm(3);
...
//   private static lastTimestamp?: number;
  ...
  private getInstance(): Logger | LoggerService | undefined {
  ...
  private static printMessage(
    ...
    const timestamp = new Date(Date.now()).toLocaleString(
        ...
        localeStringOptions,
            ...
            const pidMessage = color(`[Scaffold Me] ${process.pid}   - `);
      );*/
  ...
  async signatureTeams(urlContainer: string, containerName: string, imageName: string) {
    console.log(
      boxen(
        `Powered with  💙 by scaffoldme team\nYour app runing at ${urlContainer}\nYour docker container name is : ${containerName}\nYour docker container image name is : ${imageName}`,
