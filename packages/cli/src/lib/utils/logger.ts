import { createTaggedFormatter, CreateTaggedFormatterOptions, DEFAULT_LOGGER_HANDLERS, Logger as BaseLogger, LoggerFormatter } from '@ionic/cli-framework';
...
export function createDefaultLoggerHandlers(formatter = createFormatter()) {
  return new Set([...DEFAULT_LOGGER_HANDLERS].map((handler: any) => handler.clone({ formatter })));
}
