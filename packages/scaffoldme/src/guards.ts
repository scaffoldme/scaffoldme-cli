...
import { APIResponse, APIResponseError, APIResponseSuccess, AppAssociation, BitbucketCloudRepoAssociation, BitbucketServerRepoAssociation, CommandPreRun, CordovaPackageJson, ExitCodeException, GithubBranch, GithubRepo, GithubRepoAssociation, ICommand, IConfigScaffoldme, IntegrationName, Login, Response, SecurityProfile, Snapshot, SSHKey, StarterManifest, SuperAgentError, User } from './definitions';
...
export function isValidProject(configFile: IConfigScaffoldme): boolean {
  return configFile
    && typeof configFile.name === 'string'
    && isArray(configFile.environments);
}

export function isInstalledProject(configFile: IConfigScaffoldme): boolean {
  return existsSync(configFile.environments[0].name);
}
