...
import { ERROR_COMMAND_NOT_FOUND, Subprocess, SubprocessError, SubprocessOptions, which, WhichOptions } from '@ionic/utils-subprocess';
...
export class Shell implements IShell {
  ...
  async resolveCommandPath(command: string, options: SpawnOptions): Promise<string> {
    if (TERMINAL_INFO.windows) {
      try {
        return await this.which(command, {
  ...
  async spawn(
    command: string,
    args: readonly string[],
    { showCommand = true, ...crossSpawnOptions }: IShellSpawnOptions
  ...
  protected prepareSpawnOptions(options: IShellSpawnOptions) {
    ...
    if (!options.env) {
      throw new FatalException(`Error options`);
    }
